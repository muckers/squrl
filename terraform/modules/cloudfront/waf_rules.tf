...
resource "aws_wafv2_web_acl" "main" {
  ...
  # Rule 7: Known Bad Actors (AWS Managed Rule)
  rule {
  ...
  # Rule 8: Core Rule Set (Basic protections)
  rule {
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "squrl-cloudfront-waf-${var.environment}"
    sampled_requests_enabled   = true
  }
  ...
  tags = merge(var.tags, {
    ...
    Purpose     = "Rate limiting and abuse protection"
  })
}
...
resource "aws_cloudwatch_metric_alarm" "waf_blocked_requests" {
  ...
  tags = merge(var.tags, {
    Name        = "squrl-waf-blocked-requests-${var.environment}"
    Environment = var.environment
  })
}
...
resource "aws_cloudwatch_metric_alarm" "waf_rate_limit_triggered" {
  ...
  tags = merge(var.tags, {
    Name        = "squrl-waf-rate-limit-${var.environment}"
    Environment = var.environment
  })
}